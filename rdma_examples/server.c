/*
 * Server code
 * This is the process that sits in each cluster node 
 * providing RDMA-accessible memory to applications
 *
 * How it works:
 * Each server listens in a specific port PORT for a TCP connection.
 * Once a TCP connection is established, the server bootstraps an RDMA connection
 * with a client.
 */
...
{
    LOG(LOG_INFO,("exchange_bootstrap_data starting"));
...
int get_port_data()
{
    ...
    CHECK(retval == 0);
    CHECK_MSG(attr.active_mtu == 5, "Error: Wrong device");
...
int setup_rdma_2()
{
   LOG(LOG_INFO, ("Moving to RTR"));
   LOG(LOG_INFO, ("mtu: %d qpn: %d psn: %d lid: %d\n",
   ...
   LOG(LOG_INFO, ("Moved to RTS"));

#define DO_RECEIVE_WR
#ifdef DO_RECEIVE_WR
   LOG(LOG_INFO, ("Building receive WR"));
...
int setup_rdma_1()
{
   ...
   LOG(LOG_INFO, ("Moving QP to init"));
...
void wait_for_tcp_connection()
{
    ...
    printf("Accept'ed connection from Client: %s:%d\n",
...
int main(void)
{
    ...
    // open device, allocate protection domain
    // register memory region, create completion queues
    // create queue_pair, modify QP to INIT stage
    setup_rdma_1();
